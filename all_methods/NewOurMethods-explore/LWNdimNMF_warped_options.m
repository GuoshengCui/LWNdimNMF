function [W,H,Ht,Hc] = LWNdimNMF_warped_options(X,gnd,G,options)
...
for inmf = 1:num_nmf 
    tic 
    [Z,H,Hend,Hc,lwndimnmf_obj] = LWNdimNMF(fea,paras);W = Z;
%     [Z,H,Hc,lwndimnmf_obj] = LWNdimNMF_HAlign(fea,paras);
%     W = Z;Hend = [];
%     [Z,H,Hend,Hc,lwndimnmf_obj] = LWNdimNMF_NNnorm(fea,paras);W = Z;
    toc
    if ~isempty(Hend)
