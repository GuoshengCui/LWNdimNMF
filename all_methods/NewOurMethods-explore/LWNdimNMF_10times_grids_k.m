...
for idata = 1:length(dataname_list)
    ...
    % >= 2 views
    %{
    percentDel_list = [0.3];% 0.1,0.3,0.5 0.3,0.5,0.7,0.9
...
%     percent_pair_list = {0.3,0.5,0.7,0.9};% 0.3,0.5,0.7,0.9
...
%                 options.layers = [100,options.nSubSpace*numClust];
%                 options.layers = [200,100,options.nSubSpace*numClust];
        ...
        path = [root,'\para_LWNdimNMF2\k\',Dataname];
        if ~exist(path,'dir')
            mkdir(path);
        end
        %{
        if length(options.layers)==1
            save(strcat(path,'\',...% one layer
                num2str(percent),'_k=[',num2str(k_list(1)),...
                ',',num2str(k_list(end)),']',...
                '_alpha=',num2str(log10(options.alpha)),...
                '_beta=',num2str(log10(options.beta)),...
                '_lys_[',num2str(options.layers(1)),'].mat'),'LWNdimNMF');
        elseif length(options.layers)==2
            save(strcat(path,'\',...% two
                ...
                '_alpha=',num2str(log10(options.alpha)),...
                '_beta=',num2str(log10(options.beta)),...
                '_lys_[',num2str(options.layers(1)),',',...
