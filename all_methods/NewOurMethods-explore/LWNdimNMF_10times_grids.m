...
for idata = 1:length(dataname_list)
    ...
    % >= 2 views
    %{ 
    percentDel_list = [0.5];% 0.1,0.3,0.5
    ...
    % = 2 views
    % {
    percent_pair_list = {0.1};% 0.3,0.5,0.7,0.9 0.1,0.3,0.5
    for iper_pair = 1:length(percent_pair_list)
        ...
        options.k = 6;% 8 6 4 10 
%         options.nSubSpace = 4;
%         alpha_list = [1e-1];% 1e-2
%         beta_list = [1e0];% 1e-1
...
% %         options.k = 20; 
        options.nSubSpace = 1;
%         alpha_list = [1e-3,1e-2,1e-1,1e0];
%         beta_list = [1e-1,1e0,1e1,1e2];
%         alpha_list = [1e-3,1e-2,1e-1];
%         beta_list = [1e-1,1e0,1e1];
%         alpha_list = [1e-3,1e-2,1e-1,1e0];
%         beta_list = [1e-3,1e-2,1e-1,1e0,1e1];
%         alpha_list = [1e-3,1e-2,1e-1,1e0,1e1];
%         beta_list = [1e-3,1e-2,1e-1,1e0,1e1];
%         alpha_list = [1e-2,1e-1,1e0,1e1,1e2];
%         beta_list = [1e-2,1e-1,1e0,1e1,1e2];
%         alpha_list = [1e-1,1e0,1e1];
%         beta_list = [1e-1,1e0,1e1];
%         alpha_list = [1e-2,1e-1,1e0];
%         beta_list = [1e-1,1e0,1e1];
        num_folds = 10;%    
        repeat = 5;% 
        options.max_iter = 300;
...
%                     options.layers = [400,options.nSubSpace*numClust];
%                     options.layers = [300,200,options.nSubSpace*numClust];
%                     options.layers = [500,300,options.nSubSpace*numClust];
%                     options.layers = [800,400,100,options.nSubSpace*numClust];
...
% % % %                         X1 = normX(X1);
                    ...
                    [W,H,Ht,Hc] = LWNdimNMF_warped_options(X,gnd,G,options);
...
%                     norm_mat = repmat(sum(new_F,2),1,size(new_F,2));
        ...
        root = ['D:\my-work-space\11-imcomplete-mv-deepIMC\',...
            'all_methods\NewOurMethods-explore'];
        path = [root,'\para_LWNdimNMF2\',Dataname];
%         path = [root,'\para_LWNdimNMF_NNorm\',Dataname];
        if ~exist(path,'dir')
            mkdir(path);
        end
        %{
        if length(options.layers)==1
            save(strcat(path,'\',...% one layer
                ...
                '_[',num2str(log10(alpha_list(1))),'~',...
                num2str(log10(alpha_list(end))),']',...
                '[',num2str(log10(beta_list(1))),'~',...
                num2str(log10(beta_list(end))),']',...
                '_[',num2str(options.layers(1)),'].mat'),'LWNdimNMF');
        elseif length(options.layers)==2
            save(strcat(path,'\',...% two
                ...
                num2str(options.layers(2)),'].mat'),'LWNdimNMF');
        elseif length(options.layers)==3
            save(strcat(path,'\',...% three
                ...
                ',',num2str(options.layers(3)),'].mat'),'LWNdimNMF');
        end
        %}
    end
end
