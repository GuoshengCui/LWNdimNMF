...
%                     options.layers = [800,400,100,options.nSubSpace*numClust];
        ...
        root = ['F:\我的工作空间\11-imcomplete-mv-deepIMC\',...
            'all_methods\NewOurMethods-explore\DeepNMF'];
%         path = [root,'\para_DeepNMF\not_init_nmf\',Dataname];
        path = [root,'\para_DeepNMF\init_nmf\',Dataname];
        if ~exist(path,'dir')
            mkdir(path);
        end
        %{
        if length(options.layers)==1
            save(strcat(path,'\',...% one layer
                ...
                '_[',num2str(options.layers(1)),'].mat'),'DeepNMF');
        elseif length(options.layers)==2
            save(strcat(path,'\',...% two
                ...
                num2str(options.layers(2)),'].mat'),'DeepNMF');
        elseif length(options.layers)==3
            save(strcat(path,'\',...% three
                ...
                ',',num2str(options.layers(3)),'].mat'),'DeepNMF');
        end
        %}
    end
end
