...
% 【0.7】：'wisconsin','washington',【0.3】：'yalea','3sources3vbig'
Dataname = 'wisconsin'; 
percent = 0.7; % 2v:{0.7,0.9}; 3v:{0.1,0.3};
root = ['F:\我的工作空间\11-imcomplete-mv-deepIMC\',...
    'Figures\results_of_different_layers_of_3variants_init_nmf'];

for nlayer = 1:3
    % 1:'LWNdimNMF',2:'LWNdimNMF\_HA',3:'LWNdimNMF\_ON',4:'DeepNMF'
    for imethod = 1:4
        if imethod==1
            [knn,alpha,beta,layers] = ...
                get_paras_3layers_lwn_init_nmf(Dataname,percent,nlayer);
            path = [root,'\LWNdimNMF\',Dataname];
            Results = load_lwn(path,percent,nlayer,knn,alpha,beta,layers);
        elseif imethod==2
            [knn,alpha,layers] = ...
                get_paras_3layers_lwn_halign_init_nmf(Dataname,percent,nlayer);
        ...
        elseif imethod==4
            ...
            Results = load_deepnmf(path,percent,nlayer,knn,alpha,beta,layers);
        else
            error('imethod must be in {1,2,3,4}!');
        end
        Y(nlayer,imethod) = Results(1,2);% nmi
    end
end
figure;
Bodlabel = 22;
...
set(h(3),'FaceColor',[1.0,0.7,1.0])
legend('LWNdimNMF','LWNdimNMF\_HA','LWNdimNMF\_ON','DeepNMF',...
...
function res = load_deepnmf(path,percent,nlayer,knn,alpha,beta,layers)
    ...
    if length(layers)==1
        load(strcat(path,'\',...% one layer
            ...
            '_[',num2str(log10(alpha)),',',...
            num2str(log10(beta)),']',...
            '_[',num2str(layers(1)),'].mat'),'res_DeepNMF');
        res = res_DeepNMF{1};
    elseif length(layers)==2
        load(strcat(path,'\',...% two
            num2str(percent),'_',num2str(nlayer),'layer',...
            ...
            num2str(log10(beta)),']',...
            '_[',num2str(layers(1)),',',...
            num2str(layers(2)),'].mat'),'res_DeepNMF');
        res = res_DeepNMF{1};
    elseif length(layers)==3
        load(strcat(path,'\',...% three
            num2str(percent),'_',num2str(nlayer),'layer',...
            ...
            '_[',num2str(layers(1)),...
            ',',num2str(layers(2)),...
            ',',num2str(layers(3)),'].mat'),'res_DeepNMF');
        res = res_DeepNMF{1};
    end
end
